...
export const FeedbackButton = forwardRef<FeedbackButtonRef, FeedbackButtonProps>(({ children, onClickAsync, loadingText, successText, errorText, idleText, className, ...props }, ref) => {
  ...
  return (
    <Button className={cn(getStateStyles(), 'transition-all duration-300', className)} onClick={handleClick} disabled={state === 'loading'} {...props}>
      {getContent()}
    </Button>
  );
});
